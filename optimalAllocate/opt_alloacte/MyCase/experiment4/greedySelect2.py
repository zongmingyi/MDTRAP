...
# allocate_per_unit = {1: 1.5, 2: 1, 3: 1.5, 4: 1.8, 5: 1}
# allocate_per_unit = {1: 1.5, 2: 1, 3: 1.5, 4: 1.8, 5: 1, 6: 1.5, 7: 1, 8: 1.7, 9: 2, 10: 1}
allocate_per_unit = {1: 1.5, 2: 1, 3: 1.5, 4: 1.8, 5: 1, 6: 1.5, 7: 1, 8: 1.7, 9: 2, 10: 1, 11: 2.6, 12: 1.9, 13: 1.1,
                     14: 1.8, 15: 2.3}
with open('ratio_data_2.txt', 'r', encoding='utf8') as file:
    ...
    for k in range(IoT_SIZE):
        for j in range(BS_SIZE):
            temp_social_welfare = temp_social_welfare + \
                                  ...
                                  allocate_per_unit[j + 1] * x_i_j[k][j] * req_channel[k + 1, j + 1]
    # 比较最大的社会福利
...
# remove_first_key = sort_v_b_ratio_tuple[BS_SIZE*IoT_SIZE-1][0][0]
# remove_second_key = sort_v_b_ratio_tuple[BS_SIZE*IoT_SIZE-1][0][1]

for i in range(winner_number + 1):
    min_first_key = sort_v_b_ratio_tuple[i][0][0]
    min_second_key = sort_v_b_ratio_tuple[i][0][1]
    remove_first_key = sort_v_b_ratio_tuple[i + 1][0][0]
    remove_second_key = sort_v_b_ratio_tuple[i + 1][0][1]
    ...
    if x_var[min_first_key - 1][min_second_key - 1] == 1:
        ...
        # print("出价为%f" % bids[remove_first_key, remove_second_key])
        # print("价值为%d" % reward[remove_first_key, remove_second_key])
        user_utility = reward[min_first_key, min_second_key] * bids[remove_first_key, remove_second_key] / reward[
            remove_first_key, remove_second_key] - bids[min_first_key, min_second_key]
