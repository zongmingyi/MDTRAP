...
with open('data_200.txt', 'r', encoding='utf8') as data:
    ...
    data.close()
with open('data_400.txt', 'w+', encoding='utf8') as file:
    file.write('%d\n' % (IoT_size * 2))
    file.write('%d\n' % BS_size)
    for i in range(BS_size):
        file.write('%d\n' % (total_channel[i + 1] * 2))
    for k in range(2):
        for i in range(IoT_size):
            for j in range(BS_size):
                file.write(
                    '%d %d %.1f %d\n' % (i + 1 + 200 * k, j + 1, bids[i + 1, j + 1], request_channel[i + 1, j + 1] - 1))
    for k in range(2):
        for i in range(IoT_size):
            file.write('%d %d\n' % (i + 1 + 200 * k, reward[i + 1] + 1))
