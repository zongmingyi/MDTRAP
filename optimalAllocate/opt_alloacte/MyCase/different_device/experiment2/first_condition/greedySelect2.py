...
with open('ratio_data_1.txt', 'r', encoding='utf8') as file:
...
# print(v_b_ratio)
sort_v_b_ratio = sorted(v_b_ratio.items(), key=lambda x: x[1])
...
for i in range(BS_SIZE * IoT_SIZE - 1):
    min_first_key = sort_v_b_ratio[i][0][0]
    min_second_key = sort_v_b_ratio[i][0][1]
    remove_first_key = sort_v_b_ratio[i + 1][0][0]
    remove_second_key = sort_v_b_ratio[i + 1][0][1]
    # print(min_first_key, min_second_key, remove_first_key, remove_second_key)
